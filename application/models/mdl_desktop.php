...
class mdl_desktop extends CI_Model
{
    ...
    function get_data_search_property($search_text,$search_property_type,$search_price,$offer,$near_by)
    {   
		...
		if($near_by != ""){
		$sql .= ' AND near_by = "'.$near_by.'"';
		}
		if($offer == "1"){
		$sql .= ' AND added_date >= DATE_ADD(CURDATE(), INTERVAL -3 DAY) AND offer_details != ""';
		}
		if($search_price != ""){
			if($search_price == '1'){
    ...
    function get_data_property_price_desc($tbl_name)
    {   
        $sql = 'select * from '.$tbl_name.' where 1 ORDER BY price DESC';
        $query = $this->db->query($sql);
        return $query->result_array();
    }
	
    function get_data_property_price_asc($tbl_name)
    {   
        $sql = 'select * from '.$tbl_name.' where 1 ORDER BY price ASC';
        $query = $this->db->query($sql);
        return $query->result_array();
    }
